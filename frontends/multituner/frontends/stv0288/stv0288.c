...
static int stv0288_writeregI(struct stv0288_state *state, u8 reg, u8 data)
{
	...
	if (ret != 1)
		dprintk(0, "%s: writereg error (reg == 0x%02x, val == 0x%02x, "
...
static int stv0288_init(struct dvb_frontend *fe)
{
	struct stv0288_state *state = fe->demodulator_priv;
	int i;
	for (i=0;i<= 0x1c;i++) stv0288_writeregI(state,i,register_[i]);
...
static int
stv0288_get_delsys (struct dvb_frontend *fe, enum dvbfe_delsys *fe_delsys)
{
  dprintk (10, "%s()\n", __FUNCTION__);
...
stv0288_get_algo (struct dvb_frontend *fe)
{
  dprintk (10, "%s()\n", __FUNCTION__);
 ...
 */
static int
stv0288_get_tune_settings (struct dvb_frontend *fe,
...
static int demod_288_algo(struct dvb_frontend *fe, unsigned long freq, unsigned long baud)
{
	...
	if( flag == 1 )
	{
		dprintk(10, "Freq(%d), derot_freq(%d) ==> result_Freq(%d), Symbol(%d)\n", 
				(int)SearchFreq_Khz, 
				(int)(pIntResults_Frequency_Khz-SearchFreq_Khz),
				(int)pIntResults_Frequency_Khz,
				(int)SymbolRate_Bds );
	}
	else
