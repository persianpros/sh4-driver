/*
    tda18272.c  - Samsung NXP TDA18272 RF IC DVB-T2 Tuner
...
{ 
    ...
    bytes[0] = (u8) index;
    res |= state->config->i2c_readwrite(state->config->demod,
                                        state->config->i2c_addr,
                                        0,
                                        bytes,
                                        1);
...
static int tda18272_set_params_dvbt2(struct dvb_frontend* fe, struct dvb_frontend_parameters *params)
{
    ...
    {
        printk("%s: bandwith wrong value\n", __func__);
        return res;
    }
    
    if (res != 0)
    {
        printk("%s: error setting bandwith\n", __func__);
        return res;
    }
...
static int tda18272_init(struct dvb_frontend* fe)
{
    ...
    res |= tda18272_i2c_write_bulk(state, state->config->lt ? 
                                          tda18272_lt_on_dvb_t2 : 
                                          tda18272_lt_off_dvb_t2);

    res |= tda18272_i2c_write_bulk(state, state->config->xtout ? 
                                          tda18272_xtout_on : 
                                          tda18272_xtout_off);
    res |= tda18272_wait_irq(fe);
