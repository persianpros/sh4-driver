/*
    cxd2820 - Samsung NXP CXD2820 DVB-T2 Demod
...
static int cxd2820_set_ts_out(struct dvb_frontend* fe)
{
    ...
    res |= cxd2820_i2c_write_bulk(state, state->config->ts_out ? 
                                      cxd2820_ts_serial : 
                                      cxd2820_ts_parallel);
    return res;
}
...
static int cxd2820_set_frontend_dvbt2(struct dvb_frontend* fe, struct dvb_frontend_parameters *p)
{
    ...
    res |= cxd2820_i2c_write_bulk(state, (state->si == INVERSION_ON) ? 
                                          cxd2820_spectrum_inv_dvbt2 : 
                                          cxd2820_spectrum_nor_dvbt2);
     ...
     * We have'nt such value to differ here currently,
...
/* fixme: dvbapi5 ??? */
    ...
    if (p->u.ofdm.bandwidth == BANDWIDTH_7_MHZ)
    {
    ...
    {
/* fixme we could make a software automatism here by switching bandwith manually */
       printk("%s: not supported bandwith passed %d\n", __func__, p->u.ofdm.bandwidth);
       res = -EINVAL;
    }
