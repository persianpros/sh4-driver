/*
    TDA10024  - Samsung NXP TDA10024 DVB-C Demod
...
static int tda10024_gate_ctrl(struct dvb_frontend* fe, int enable)
{
    ...
    res = tda10024_i2c_write_bulk(state , enable ? tda10024_gate_on : 
                                              tda10024_gate_off);
...
static int tda10024_read_snr(struct dvb_frontend* fe, u16* snr)
{
    ...
    *snr=0;
    ...
    if (state->mod == QAM_16)
       *snr = (u16) tda10024_lookup_snr(bytes[0], tda10024_snr_lut_qam16, 
                                         sizeof(tda10024_snr_lut_qam16));

    if (state->mod == QAM_32)
       *snr = (u16) tda10024_lookup_snr(bytes[0], tda10024_snr_lut_qam64, 
                                             sizeof(tda10024_snr_lut_qam64));
    ...
    if (state->mod & (QAM_128 | QAM_32))
       *snr = (u16) tda10024_lookup_snr(bytes[0], tda10024_snr_lut_qam256, 
                                             sizeof(tda10024_snr_lut_qam256));
...
static struct dvbfe_info dvbc_info = {
    .name = "Samsung NXP TDA10024 DVB-C",
...
static int tda10024_set_frontend(struct dvb_frontend* fe, struct dvb_frontend_parameters *p)
{
    ...
    if (res != 0)
    {
        printk("%s: ups setting power failed %d\n", __func__, res);
    }
