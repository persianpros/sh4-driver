...
//	set_bit(KEY_LEFT,  et6226_data->keyboard->keybit);
//	set_bit(KEY_RIGHT, et6226_data->keyboard->keybit);
//	set_bit(KEY_POWER, et6226_data->keyboard->keybit);
...
int __init fortis_4g_if_init(void)
{
	...
	et6226_data->display_enable = et6226_init->ctl_reg && ET6226_DISPLAY_ENABLE;
...
int fortis_4gSetLED(int which, int level)
{
	...
	{
		printk("[fortis_4g_i2c] LED number %d out of range (1-%d)\n", which, MAX_LED);
		return -EINVAL;
	}
	...
	else
	{
		if (MAX_LED_BRIGHT != 1)
		{
			...
			{
				printk("[fortis_4g_i2c] LED brightness %d out of range (0-%d)\n", which, MAX_LED_BRIGHT);
				return -EINVAL;
			}
		}
		else
...
{
	int ret = -1;
	int display_enable = et6226_data->display_enable;
	...
	else
	{
		...
		if (level > 0 && !et6226_data->sleep_mode)
		{
			display_enable = ET6226_DISPLAY_ENABLE;
		}
		et6226_data->brightness = level;
		dprintk(20, "Display on, chip brightness level %d\n", level);
	}
	et6226_data->display_enable = display_enable;

	ret = et6226_i2c_write(et6226_data->client, ET6226_CTL_ADDR, ET6226_CTL_DATA(display_enable, et6226_data->sleep_mode, et6226_data->display_mode, et6226_data->brightness));
